...
def replace_with_dictionary(a_string, dict_per, ignorecase=True, metadata=None):
    ...
    # Try to retrieve original lettercase in trivial cases
    case_match = True
    if ignorecase and sum(cases) > 0:
        if len(cases) == len(new_string):
            new_string = retrieve_case(new_string, cases)
            print(
                f"Case retrieved: {og_string} >>> {new_string}. num upper cases: {sum(cases)}"
            )
        elif sum(cases) == 1 and cases[0]:
            # easy case 1st letter capitalization
            new_string = new_string.capitalize()
            print(
                f"Case capitalized: {og_string} >>> {new_string}. num upper cases: {sum(cases)}"
            )
        else:
            print(
                f"Debug: Case issue - {og_string} >>> {new_string}. num upper cases: {sum(cases)}"
            )
            case_match = False
...
def merge_letters(not_fixed, pattern):
    not_fixed_merged = []
    ...
    for i in range(len(pattern)):
        if not pattern[i]:
            if create_new:
                not_fixed_merged.append([])
            create_new = False
            not_fixed_merged[-1].append(not_fixed.pop(0))
        else:
            create_new = True

    not_fixed_merged = [x for y in not_fixed_merged for x in y]
    not_fixed_merged = ["".join(string) for string in not_fixed_merged]

    return not_fixed_merged


def remove_fixed(a_string, fixed_mapping):
    og_string = a_string

    keys = list(fixed_mapping.keys())
    index = 0
    not_fixed = []
    all_fixed = []
    pattern = np.zeros(len(a_string))
    ...
    while index < len(a_string):
        ...
        if final_key == "":
            step_size = 1
            not_fixed.append(a_string[index])

        else:
            step_size = len(final_key)
            all_fixed.append(final_key)
            pattern[index : index + len(final_key)] = 1

        index += step_size

    not_fixed_merged = merge_letters(not_fixed, pattern)
...
def obfuscate_text(
    ...
    for b in bold_texts:
        ...
        if fixed_mapping:
            new_text_segs = remove_fixed(new_text, fixed_mapping)

            for a_seg in new_text_segs:
                b_swapped, case_preserved = replace_with_dictionary(
                    a_seg, mapping, metadata=metadata, ignorecase=ignorecase
                )
                ...
                obfuscated = obfuscated.replace(a_seg, b_swapped)
...
def create_obf_dictionary(
    ...
    fixed_dict = {}
    for col in fix_cols:
        if str(df_sub[col].item()).lower() != "nan":
            fixed = df_sub[col].item().lower().split(",")
            fixed_dict = fixed_dict | {v: [[[v]]] for v in fixed}
            fixed_dict = fixed_dict | {v.title(): [[[v.title()]]] for v in fixed}

    return obf_dict, fixed_dict
    ...
    for root, d, files in os.walk(formatted_path):
        for f in sorted(files):
            if os.path.basename(f).split(".")[-1] == "json":
                ...
                try:
                    obf_dict, fixed_dict = create_obf_dictionary(obf_df, q_num=q_num)
                    mappings = sample_obf(
